...
% FIXME: TODO: discuss single precision
...
CHOLMOD 1.0 replaces {\tt chol} (the sparse case), {\tt symbfact}, and {\tt
etree} in MATLAB 7.2 (R2006a), and is used for {\tt x=A}$\backslash${\tt b}
when {\tt A} is symmetric positive definite \cite{GilbertMolerSchreiber}.  It
will replace {\tt sparse} in a future version of MATLAB.
...
%-------------------------------------------------------------------------------
\newpage \section{Single-precision sparse matrix support}
%-------------------------------------------------------------------------------

{\bf CHOLMOD v5.0.0}: introduces the first part of support for single
        precision sparse matrices, with the introduction of the new
        CHOLMOD:Utility Module.  The CHOLMOD:Utility Module replaces the
        CHOLMOD:Core Module that appeared in prior versions of CHOLMOD.
        Single precision is not yet incorporated into the remaining Modules,
        however.  This feature will be implemented soon in a later version
        of CHOLMOD.
...
\item {\tt cholmod\_sparse}:  a sparse matrix in compressed-column form,
	...
	\item {\tt A->z}, a {\tt double} % FIXME
            array of size {\tt A->nzmax} if {\tt A} is zomplex.
...
\item {\tt SuiteSparse\_config}:
    ...
    or co-authored by Davis are configured.
\end{itemize}
...
METIS 5.1.0 is optional; a copy of it is in {\tt SuiteSparse\_metis}.
...
SuiteSparse now has a complete \verb'cmake'-based build system.
Each package (SuiteSparse\_congig, AMD, CAMD, CCOLAMD, CAMD, and CHOLMOD) has its own
\verb'CMakeLists.txt'.  Use \verb'cmake' to build each package in
that order.

An optional \verb'Makefile' is provided at the top-level of \verb'SuiteSparse'
Type {\tt make} in that directory.  The AMD,
COLAMD, CAMD, CCOLAMD, and {\tt CHOLMOD} libraries will be compiled.
To compile and run demo programs for each package,
type \verb'make demos'.  For CHOLMOD, the residuals should all be small.
...
Each library has its own \verb'Find*.cmake' script to use in
the \verb'cmake'
\verb'find_library' command.

To install CHOLMOD in default locations use
{\tt make install}.
...
CHOLMOD includes OpenMP acceleration for some operations.
In CHOLMOD versions prior to v6.0.0, the number of threads to use was controlled
by a compile time parameter.  This is now replaced with run-time controls.
...
preprocessor macro \verb'ENABLE_CUDA' defined.  It is enabled by default
but can be disabled by setting this to false when using cmake.
...
{\tt cholmod\_l\_} routines use {\tt int64\_t}.  All floating-point
values are {\tt double}.    % FIXME
...
is a {\tt double} % FIXME
array {\tt x} of size {\tt 2*n}, with the real and imaginary
parts interleaved (the real part comes first, as a {\tt double}, % FIXME
followed the
imaginary part, also as a {\tt double}.  % FIXME
Thus, the real part of the {\tt k}th
entry is {\tt x[2*k]} and the imaginary part is {\tt x[2*k+1]}.

A zomplex matrix of size {\tt n} stores its real part in one
{\tt double} % FIXME
array of size {\tt n} called {\tt x} and its imaginary part
in another {\tt double} % FIXME
array of size {\tt n} called {\tt z} (thus the
...
For {\tt int64\_t} integers, the statement would become:
\begin{verbatim}
...
To obtain the name of the {\tt int64\_t} version of the same routine, simply
...
of {\tt int64\_t} routines), divided into a set of inter-related
...
routines, the {\tt Utility} Module, and the AMD and COLAMD packages.  They
...
The {\tt Utility} module can create an identity Cholesky factorization ($\m{LDL}\tr$ where
$\m{L}=\m{D}=\m{I}$) that can then be modified by these routines.
Requires the {\tt Utility} module.  Not required by any other CHOLMOD Module.
...
Requires the {\tt Utility} Module, and two external packages: LAPACK and the BLAS.
...
dissection.
Requires the {\tt Utility} and {\tt Cholesky} Modules, and two packages: {\tt METIS 5.1.0}, CAMD, and CCOLAMD.
...
% FIXME: also uses double when the method is single
...
respectively, and a {\tt double} % FIXME
array {\tt Xwork} of size
{\tt xworksize} entries.  The {\tt Head} array is normally equal to -1
...
%---------------------------------------
\subsection{{\tt cholmod\_transpose\_unsym}: transpose/permute unsymmetric sparse matrix}
%---------------------------------------
...
\item simplicial symbolic:  Consists of two size-{\tt n} arrays: the fill-reducing
    ...
    {\tt L->ColCount} may be exact (obtained from the analysis routines), or it
    may be a guess.  During factorization, and certainly after update/downdate,
    the columns of {\tt L} can have a different number of nonzeros.  {\tt
    L->ColCount} is used to allocate space.  {\tt L->ColCount} is exact for the
...
\item simplicial numeric:  These represent {\tt L} in a compressed column form.  The
    ...
    \item $\m{LDL}\tr$:	{\tt L} is unit diagonal.  Row indices in column {\tt j} are located in
        ...
        parts, and is of size {\tt 2*lnz*sizeof(double)}.  % FIXME
        For the zomplex case,
        {\tt L->x} is of size {\tt lnz*sizeof(double)} % FIXME
        and holds the real part; {\tt L->z}
...
\item supernodal symbolic:  A representation of the nonzero pattern of the
    ...
    and is of size \newline
    {\tt 2*L->xsize*sizeof(double)}.  % FIXME
    The zomplex supernodal case
...
\item supernodal numeric:  Always an $\m{LL}\tr$ factorization.  {\tt L} has a non-unit
     ...
     {\tt 2*L->xsize*sizeof(double)}.  % FIXME
     The zomplex supernodal case is not
