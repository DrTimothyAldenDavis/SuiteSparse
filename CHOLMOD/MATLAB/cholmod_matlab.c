//------------------------------------------------------------------------------
// CHOLMOD/MATLAB/cholmod_matlab.c: utilities for CHOLMOD's MATLAB interface
//------------------------------------------------------------------------------
...
// Create a read-only shallow or deep CHOLMOD copy of a MATLAB sparse matrix.
// A is returned with A->xtype of CHOLMOD_PATTERN (if Amatlab is logical),
// CHOLMOD_REAL (if Amatlab is real), or CHOLMOD_COMPLEX (if Amatlab is
// complex).  The A->p and A->i arrays are always shallow.  The numerical array
// A->x can be shallow or deep, and must be freed by sputil_free_sparse2.  The
// CHOLMOD matrix A must not be modified, except by sputil_free_sparse2.
//
// Example:
//
//      mxArray M = pargin [0] ;
...
//      size_t xsize ;
//      A = sputil_get_sparse2 (M, -1, CHOLMOD_SINGLE, &Amatrix, &xsize, cm) ;
//      ... use A in CHOLMOD; do not change A or xsize
//      sputil_free_sparse2 (A, xsize, cm) ;
...
// Frees any content of a CHOLMOD sparse matrix created by sputil_get_sparse2.

void sputil_free_sparse2    // free a matrix created by sputil_get_sparse2
(
    // input/output:
    cholmod_sparse *A,      // a CHOLMOD matrix created by sputil_get_sparse2
    // input:
...
// Create a read-only shallow or deep CHOLMOD copy of a MATLAB dense matrix.
// X is returned with X->xtype CHOLMOD_REAL (if Xmatlab is real), or
// CHOLMOD_COMPLEX (if Xmatlab is complex).  The numerical array X->x can be
// shallow or deep, and must be freed by sputil_free_dense2.  The CHOLMOD
// matrix X must not be modified, except by sputil_free_dense2.  Only
// single/double real/complex matrices are supported.
//
// Example:
//
//      mxArray M = pargin [0] ;
//      cholmod_dense *X, Xmatrix ;
//      size_t xsize ;
//      X = sputil_get_dense2 (M, CHOLMOD_SINGLE, &Mmatrix, &xsize, cm) ;
//      ... use X in CHOLMOD; do not change X or xsize
//      sputil_free_dense2 (X, xsize, cm) ;
...
// Frees any content of a CHOLMOD dense matrix created by sputil_get_dense2.
...
)
{
    ...
    if (!mxIsSparse (Amatlab))
    {
        ...
        else if (mxIsLogical (Amatlab))
        {
            ...
            char *x ;
            int64_t *Ap, *Ai ;
            int64_t nrow, ncol, i, j, nz, nzmax, p ;
...
int64_t sputil_nelements (const mxArray *arg)
{
    double size ;
    const int64_t *dims ;
    int64_t k, ndims ;
    ndims = mxGetNumberOfDimensions (arg) ;
    dims = (int64_t *) mxGetDimensions (arg) ;
    size = 1 ;
    for (k = 0 ; k < ndims ; k++)
    {
        size *= dims [k] ;
...
)
{
    int64_t *Ap, *Ai, *Zp, *Zi ;
    double *Ax, *Az, *Zx ;
    int64_t j, p, nzeros = 0, is_complex, pz, nrow, ncol ;
    cholmod_sparse *Z ;
...
)
{
    ...
    int64_t *Sp, *Si ;
    double *Sx, *Sz ;
    int64_t pdest, k, ncol, p, pend, nz ;
...
/* copy i or j arguments into an int64_t vector.  For small integer types, i and
...
)
{
    ...
    {
        /* copy double input into int64_t vector (convert to 0-based) */
        ok = TRUE ;
        ok2 = TRUE ;
        ok3 = TRUE ;
        n2 = 0 ;
...
{ \
    stype *Sx ; \
    Sp = (int64_t *) mxGetJc (S) ; \
    Si = (int64_t *) mxGetIr (S) ; \
    ...
    else
    {
        ...
        else switch (s_class)
        {
            ...
            case mxUINT64_CLASS:
                ...
                for (k = 0 ; k < nz ; k++)
                {
                    Tx [k] = (double) (((uint64_t *) x_vector) [k]) ;
                }
                break ;
...
mxArray *sputil_copy_sparse (const mxArray *A)
{
    ...
    double *Ax, *Az, *Sx, *Sz ;
    int64_t *Ap, *Ai, *Sp, *Si ;
    int64_t anz, snz, p, j, nrow, ncol, pend ;
...
mxArray *sputil_sparse_to_dense (const mxArray *S)
{
    ...
    double *Sx, *Sz, *Xx, *Xz ;
    int64_t *Sp, *Si ;
    int64_t nrow, ncol, i, j, p, pend, j2 ;
...
)
{
    ...
    int64_t nrow, ncol, k, nz, i_is_scalar, j_is_scalar, s_is_sparse,
