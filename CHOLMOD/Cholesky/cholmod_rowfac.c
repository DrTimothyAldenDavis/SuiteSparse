...
// Iwork (nrow)
//
// Supports any xtype and dtype, except a pattern-only input matrix A cannot be
// factorized.
...
{                                                                           \
    ...
    {                                                                       \
        ...
        {                                                                   \
            ...
            parent = PARENT (i) ;   /* traverse up the etree to parent */   \
        }                                                                   \
...
//------------------------------------------------------------------------------
// t_cholmod_rowfac template
//------------------------------------------------------------------------------

#define DOUBLE

#define REAL
#include "t_cholmod_rowfac_worker.c"
#define COMPLEX
#include "t_cholmod_rowfac_worker.c"
#define ZOMPLEX
#include "t_cholmod_rowfac_worker.c"
...
(
    // input:
    cholmod_sparse *A,  // matrix to analyze
    Int *Fi,            // nonzero pattern of kth row of A', not required
...
// This is meant for use in LPDASA only.  For general usage, see
// cholmod_rowfac.
...
)
{
    ...
    if (L->xtype != CHOLMOD_PATTERN &&
        (A->xtype != L->xtype || A->dtype != L->dtype))
    {
        ERROR (CHOLMOD_INVALID, "xtype or dtype of A and L do not match") ;
        return (FALSE) ;
    }
    if (L->is_super)
    {
    ...
    CHOLMOD(alloc_work) (n, n, s, A->dtype, Common) ;
    if (Common->status < CHOLMOD_OK)
    {
        return (FALSE) ;
    }
    ASSERT (CHOLMOD(dump_work) (TRUE, TRUE, A->nrow, A->dtype, Common)) ;
