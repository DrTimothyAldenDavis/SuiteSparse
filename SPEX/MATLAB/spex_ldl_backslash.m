...
% spex_ldl_backslash: computes the exact solution to the sparse linear system Ax =
...
% Usage:
%
% x = spex_ldl_backslash (A,b) returns the solution to Ax=b using default settings.
%
...
% Example:
%
%   % In this first example, x = spex_ldl_backslash (A,b) returns an approximate
%   % solution, not because it was computed incorrectly in spex_ldl_backslash.  It
...
% spex_ldl_backslash is a wrapper for the exact routines contained within the SPEX
% software package.  In order to use spex_ldl_backslash you must install the MATLAB
