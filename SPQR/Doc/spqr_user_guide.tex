...
feature of the MATLAB \verb'qr' in version R2008a and earlier
...
\begin{itemize}
    ...
    \item \verb'opts.ordering': a string describing what ordering method to
    use.  Let \verb'[m2 n2]=size(S)' where \verb'S' is obtained by removing
    singletons from \verb'A'.  The singleton permutation places \verb'A*P' in
    the form \verb'[A11 A12 ; 0 S]' where \verb'A11' is upper triangular with
    diagonal entries all greater than \verb'opts.tol'.
...
%-------------------------------------------------------------------------------
\subsection{Examples how to use the MATLAB interface}
%-------------------------------------------------------------------------------
...
\begin{tabular}{|l|l|}
    ...
    \verb"[Q,R,P] = spqr (A',struct('Q','Householder')) ;"
        & low fill-in in \verb'R', \verb'Q' in\\
    \verb"x = spqr_qmult (Q,R'\(P'*b),1) ;"
        & efficient Householder form \\
    \hline
...
edit the \verb'SuiteSparse/UFconfig/UFconfig.mk' configuration file.  The
...
\verb'UFconfig.mk' file use \verb'-llapack' and \verb'-lblas'; the latter may
...
\verb'UFconfig.mk' file.

Four compile-time options can be used to modify how SuiteSparseQR is compiled.
Select these via the \verb'SPQR_CONFIG=' line in the \verb'UFconfig.mk' file.
...
\begin{itemize}
    ...
    \item \verb'-DTIMING': enable wall clock timing of the analysis,
    factorization, and solve phases, and exact flop counts.  This is for Linux
    only.  Timing is disabled by default.
...
\verb'UF_long' integer, defined in the \verb'SuiteSparse/UFconfig/UFconfig.h'
file.  This integer is \verb'long' except on a Windows-64 platform for which it
is the  \verb'__int64' type.  The intent of \verb'UF_long' is that it should be
32-bits on a 32-bit platform, and 64-bits on a 64-bit platform.  SuiteSparseQR
only provides a \verb'UF_long' version.  Most users (except Windows-64) can
simply use \verb'long' as the basic integer type passed to and returned from
SuiteSparseQR.
...
\verb'SPQR_istat' which is \verb'UF_long', \verb'cc->memory_usage' which is
...
is available at
\htmladdnormallink{http://www.cise.ufl.edu/research/sparse}{http://www.cise.ufl.edu/research/sparse}
