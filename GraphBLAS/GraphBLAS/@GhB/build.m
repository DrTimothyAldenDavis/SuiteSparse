...
%
% C = GhB.build (I, J, X, m, n, dup, type, desc)
%
% GhB.build constructs an m-by-n sparse matrix C from a list of entries,
...
% types).  If the type is not specified, it defaults to the type of X.
%
% The integer arrays I and J may be double, int64, or uint64: If I, J, and X
% are double, the following examples construct the same sparse matrix S:
%
%   S = sparse (I, J, X) ;
%   S = GhB.build (I, J, X, struct ('kind', 'sparse')) ;
%   S = double (GhB.build (I, J, X)) ;
%   S = double (GhB.build (uint64(I), uint64(J), X)) ;
%
% The row and column indices I and J need not be in any particular order, but
% GhB.build is fastest if I and J are provided in column-major order if
% building a built-in sparse matrix.  If desc.format is 'by row', then
% GhB.build is fastest if I and J are in row-major order.
%
% If desc.base is 'zero-based', and I and J are int64 or uint64, then I and J
% are treated as zero-based, where (0,0) is the first entry in the top left of
% S, and (m-1,n-1) is the position in the bottom right corner of S.  GhB.build
% is fastest if I and J are int64 or uint64, and desc.base is 'zero-based'.
% The default is the same as built-in indexing, which is 'one-based'.
% desc.base is ignored if I and J are double.
%
% If I, J, and/or X are scalars, and any of I, J, or X is a vector of length e,
% the scalars are expanded into vectors of length e.  Any vectors I, J, and X
% must all be the same size, e.
%
% To build an iso-valued matrix, X must a scalar, and the dup operator must be
% one of: '1st', '2nd', 'any', 'min', 'max', 'or', 'and', 'bitor', 'bitand', or
% dup may be omitted if X is logical.
%
