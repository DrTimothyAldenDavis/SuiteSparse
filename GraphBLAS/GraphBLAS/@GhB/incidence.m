...
% C = GhB.incidence (A) is the graph incidence matrix of the square matrix A.
...
% ignored.
%
%   C = GhB.incidence (A, ..., 'directed') constructs a matrix C of size n-by-e
%       where e = GhB.entries (GhB.offdiag (A)).  Any entry in the upper or
...
%
%   C = GhB.incidence (A, ..., 'undirected') assumes A is symmetric, and only
...
%       'double'.  Unsigned types are not allowed since C must contain -1's.
%
% Examples:
%
%   A = sprand (5, 5, 0.5)
