...
)
{
    ...
    double hyperratio ;
    GxB_Global_Option_get (GxB_HYPER, &hyperratio) ;
    pargout [4] = mxCreateDoubleScalar (hyperratio) ;

    char *name ;
    GxB_Global_Option_get (GxB_LIBRARY_NAME, &name) ;
    ...
    char *about ;
    GxB_Global_Option_get (GxB_LIBRARY_ABOUT, &about) ;
    pargout [8] = mxCreateString (about) ;

    char *license ;
    GxB_Global_Option_get (GxB_LIBRARY_LICENSE, &license) ;
    ...
    int api [3] ;
    GxB_Global_Option_get (GxB_API_VERSION, api) ;
    ...
    char *api_about ;
    GxB_Global_Option_get (GxB_API_ABOUT, &api_about) ;
