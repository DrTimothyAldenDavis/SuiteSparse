...
\verb'GRAPHBLAS_USE_NIT' option \verb'OFF' in the cmake build options.  Good
...
\verb'FactoryKernels', enable the \verb'GRAPHBLAS_COMPACT' option in the cmake
build options.  By default, \verb'GRAPHBLAS_COMPACT' is off, to enable the
\verb'FactoryKernels'.
...
feasible for a more targetted application such as RedisGraph.
...
nothing.  If OpenMP is not available or \verb'-DNOPENMP=1' is used, then
...
\begin{verbatim}
    #include "GraphBLAS.h"
    #undef I    /* complex.h #define's I, used below as an array */
    #include <stdio.h>
    int main (void)
    {
