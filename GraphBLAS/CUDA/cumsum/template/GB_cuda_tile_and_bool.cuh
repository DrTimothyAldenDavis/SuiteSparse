//------------------------------------------------------------------------------
// GraphBLAS/CUDA/template/GB_cuda_tile_and_bool.cuh:  warp-level reductions
//------------------------------------------------------------------------------
...
// a simple tile-wide AND reduction, not a cumsum.

//------------------------------------------------------------------------------
// GB_cuda_tile_and_bool: reduce a bool value across a single warp with AND
//------------------------------------------------------------------------------

// On input, each thread in the tile holds a single bool value.  On output,
// thread zero holds the AND of values from all the warps.

__device__ __inline__ bool GB_cuda_tile_and_bool
(
...
)
{
    ...
    // Note that only thread 0 will have the full AND of all values in
