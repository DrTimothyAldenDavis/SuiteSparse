...
)
{
    ...
    int stats2 [CCOLAMD_STATS] ;/* stats for csymamd */

    int *cp, *cp_end, result, nnz, col, length, ok ;
    int *stats ;
    stats = stats2 ;
    ...
    */
    ...
    ok = csymamd (n_col, A, p, perm, knobs, stats, &mxCalloc, &mxFree,
	    NULL, -1) ;
