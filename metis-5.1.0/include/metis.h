...
--------------------------------------------------------------------------------
This file appears as SuiteSparse/metis-5.1.0/metis.h.  This copy of metis-5.1.0
is unchanged from the original distribution, with the exception of this file,
and minor modifications in libmetis/parmetis.c and libmetis/balance.c to
avoid a compiler warning.

The default integer size has been changed from 32 bits to 64 (IDXTYPEWIDTH).
Modified by Tim Davis, Jan 11, 2016, for SuiteSparse 4.5.0 (and subsequent
versions).  You may use the original METIS 5.1.0, with 32-bit integers.  When
compiled, SuiteSparse will properly detect the size of the METIS idx_t, and it
will work with both versions.  The only constraint is that SuiteSparse will not
be able to use METIS on the largest problems, if you limit its version of METIS
to 32-bit integers.

The abs and iabs functions in the original libmetis/parmetis.c and
libmetis/balance.c give compiler warnings with this change, so they have been
replaced with a type-agnostic macro, ABS.
