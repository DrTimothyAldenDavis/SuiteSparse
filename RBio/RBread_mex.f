...
c	----------------------------------------------------------------
	...
	integer
     ...
     $	    skind, cmplex, wmat, cpmat, Zmat, nw,
...
c	----------------------------------------------------------------
	...
	call RBint (iclass)
	nw = max (nrow,ncol) + 1
	wmat = mxCreateNumericMatrix (nw, 1, iclass, 0)
...
c	----------------------------------------------------------------
	...
	if (mkind .eq. 2) then
c	    complex matrices
	    Cmat = mxCreateNumericMatrix (2 * nnz1, 1,
...
c	    ------------------------------------------------------------
	    ...
	    pargout (2) = mxCreateSparse (nrow, ncol, max (nzeros,1), 0)
