...
%-------------------------------------------------------------------------------
    ...
    \item \verb'PARU_CONTROL_PRESCALE': % prescale input matrix, or not
        If zero, no scaling is performed.  If nonzero, each row is scaled by
...
\begin{itemize}
    ...
    \item \verb'opts.prescale':  if true, pre-scale the input matrix (default:
    true).  The prescaled matrix is $RA$ where
