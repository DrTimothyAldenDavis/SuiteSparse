...
\subsubsection{{\sf ParU\_Info}: return values of each ParU method}
    ...
    user application should check the output before continuing.
...
\subsubsection{{\sf ParU\_Control}: parameters that control ParU}
    ...
    \verb'paru_max_threads' & default: $0$.  Initialized with \verb'omp_max_threads' \\
    \hline
    ...
    \verb'panel_width' & default: 32. Width of panel for dense factorization \\
    ...
    The first row of the options in the table above is used in both the
    symbolic analysis and numerical factorization.  The second row of the
    options is used in the symbolic analysis.  The third row of control options
    shows those that have an impact on numerical factorization.

    If set to zero, \verb'paru_max_threads' is initalized by
...
\subsubsection{{\sf ParU\_LSolve}: solve $Lx=b$}
...
\subsubsection{{\sf ParU\_USolve}: solve $Ux=b$}

    \verb'ParU_USolve' solves a lower triangular system, $Ux=b$ with vectors
...
\subsubsection{{\sf ParU\_Perm}: permute and scale a dense vector or matrix}
...
\subsubsection{{\sf ParU\_InvPerm}: permute and scale a dense vector or matrix}
...
\subsubsection{{\sf ParU\_Residual}: compute the residual}
    ...
    The \verb'ParU_Residual' function comptes the relative residual of
...
\subsubsection{{\sf ParU\_FreeSymbolic}: free a symbolic analysis}
