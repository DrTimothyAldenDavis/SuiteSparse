...
you need to link your program with all libraries, either the static
ones:
...
or the shared versions of the same libraries (with {\tt *.so} filetypes
on Linux/Unix; {\tt *.dylib} on the Mac).
...
If you use
the shared versions of the same libraries (with {\tt *.so} filetypes
on Linux/Unix; {\tt *.dylib} on the Mac), and install them in
/usr/local/lib and /usr/local/include, then the compiler will know
